...
const CandidateSearch = (props) => {
  ...
  const fetchCandidate = () => {
    if (location.state !== null) {
      ...
      axios.get("main/getCandidate", { params: value }).then((res) => {
        const arr = removeDuplicates(res.data, 'party')
        const constitute = removeDuplicates(res.data, 'constituency')
        setLoader(false)
  ...
  const handleSubmit = () => {
    ...
    axios.get("main/getCandidate", { params: value }).then((res) => {
      ...
      const arr = removeDuplicates(res.data, 'party')
      const constitute = removeDuplicates(res.data, 'constituency')
      // Array.from(new Set(constituencies))
      setLoader(false)
      setConstituency(constitute.sort())
      setParties(arr.sort())
  ...
  useEffect(() => {
    if (selectedConstituency) {
      ...
      if (data.length === 0) {
        ...
        setConstituency(removeDuplicates(candidateList.filter((items) => items.party === selectedParty), 'constituency'))
      ...
      const removedup = removeDuplicates(candidateList.filter((items) => items.party === selectedParty), 'constituency')
      setCandidateFilter(candidateList.filter((items) => items.party === selectedParty))
      setConstituency(removedup)
    }
  }, [selectedParty])
  ...
  const handleConstituency = (e) => {
    ...
    if (selectedParty) {
      ...
      setParties(removeDuplicates(candidateList.filter((items) => items.constituency === e), 'party'))
      setCandidateFilter(candidateList.filter((items) => items.constituency === e))
    }
  }
