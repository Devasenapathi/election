...
router.get('/delete', async function (req, res) {
	try {
		console.log("hello")
		var ress = await Election.deleteMany({ state: "TAMIL NADU" })
		console.log(ress)
	} catch {
...
router.get("/main", function (req, res) {
	...
	fs.createReadStream("C:/electionweb/server/Election_dataset/TamilNadu.csv") // Replace 'data.csv' with your CSV file's path
		.pipe(csv())
		.on("data", (data) => {
			results.push(data);
		})
		.on("end", async () => {
			...
			console.log(results[0]['State'].toUpperCase(),'State name')
